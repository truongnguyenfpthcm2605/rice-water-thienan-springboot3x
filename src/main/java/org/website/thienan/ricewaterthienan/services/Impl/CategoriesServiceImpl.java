...
public class CategoriesServiceImpl implements CategoriesService {
    ...
    @Override
    @Caching(
            put = {@CachePut(cacheNames = "category", key = "#CategoriesRequest.id")},
            evict = {
                @CacheEvict(cacheNames = "categories", allEntries = true),
                @CacheEvict(cacheNames = "categoriesActive", allEntries = true)
            })
    ...
    public Optional<Categories> findById(Integer id) {
        Categories categories = categoriesRepository
                .findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Not found Categories ID : " + id));
    ...
    public Optional<Categories> findByName(String name) {
        Categories categories = categoriesRepository
                .findByName(name)
                .orElseThrow(() -> new ResourceNotFoundException("Not found Categories Name : " + name));
        return Optional.of(categories);
    }
