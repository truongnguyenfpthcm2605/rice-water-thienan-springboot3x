...
public class SettingController {
    ...
    public ResponseEntity<MessageResponse> findById(@Valid @NotNull @PathVariable String id) {
        ...
        Setting setting =
                settingRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Setting not found"));
        return new ResponseEntity<>(
                MessageResponse.builder()
                        .code(HttpStatus.OK.value())
                        .timeStamp(LocalDateTime.now())
    ...
    public ResponseEntity<MessageResponse> save(@Valid @RequestBody Setting setting) {
        ...
        return new ResponseEntity<>(
                MessageResponse.builder()
                        ...
                        .data(settingRepository.save(setting))
                        .build(),
                HttpStatus.OK);
    }
    ...
    public ResponseEntity<MessageResponse> update(@Valid @RequestBody Setting settingR) {
        ...
        Setting setting = settingRepository
                .findById(settingR.getId())
                .orElseThrow(() -> new ResourceNotFoundException("Not found Setting"));
