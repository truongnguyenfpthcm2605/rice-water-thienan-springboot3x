...
public class OrdersController {
    ...
    public ResponseEntity<MessageResponse> findByKeyWord(
            @Valid @NotNull @RequestParam(required = false, defaultValue = "") String keyword,
            @NotNull @PathVariable Optional<Integer> page,
            @NotNull @PathVariable String sort) {
    ...
    public ResponseEntity<MessageResponse> findByStatus(
            @Valid @NotNull @RequestParam(defaultValue = "", required = false) String status,
            @NotNull @PathVariable Optional<Integer> page,
            @NotNull @PathVariable String sort) {
    ...
    public ResponseEntity<MessageResponse> findByName(@Valid @NotNull @PathVariable String name) {
        ...
        Orders orders = ordersService
                .findByName(name)
                .orElseThrow(() -> new ResourceNotFoundException("Not found Orders Phone " + name));
        return new ResponseEntity<>(
                MessageResponse.builder()
                        .code(HttpStatus.OK.value())
                        .timeStamp(LocalDateTime.now())
    ...
    public ResponseEntity<MessageResponse> save(
        ...
        orders.setAccount(accountServices
                .findById(ordersRequest.getAccountId())
                .orElseThrow(
                        () -> new ResourceNotFoundException("Not found Account ID " + ordersRequest.getAccountId())));
        ...
        for (OrdersDetailRequest e : ordersDetailRequests) {
            ...
            orderDetail.setProduct(productService
                    .findById(e.getProductId())
                    .orElseThrow(() -> new ResourceNotFoundException("Not found Product ID " + e.getProductId())));
    ...
    public ResponseEntity<MessageResponse> update(
            @Valid @NotNull @PathVariable String id, @RequestBody OrdersRequest OrdersRequest) {
        log.info("update: {}", OrdersRequest.getName());
        Orders orders =
                ordersService.findById(id).orElseThrow(() -> new ResourceNotFoundException("Not found Orders ID" + id));
        orders.setPhone(OrdersRequest.getPhone());
        orders.setName(OrdersRequest.getName());
        orders.setAddress(OrdersRequest.getAddress());
        orders.setStatus(OrdersRequest.getStatus());
